	...
	public void handle(CommandContext<Player> context){
		...
		if (!combatManager.hasTags(target)) {
			if (target.equals(player)) {
				...
				messenger.message(player, Translations.COMMAND_TAG_NO_TAGS_OTHER);
			}
		} else {
			if (target.equals(player)) {
				messenger.message(player, Translations.COMMAND_TAG_INFO_SELF);
			} else {
				messenger.message(player, Translations.COMMAND_TAG_INFO_OTHER);
			}
			for (CombatTag tag : combatManager.getTags(target)){
				...
				if (opposite instanceof BlockCombatUser blockCombatUser){
					messenger.message(player, Translations.COMMAND_TAG_INFO_BLOCK, Placeholder.of("who", blockCombatUser.getBlock().getType().getKey().toString()),
					...
					messenger.message(player, Translations.COMMAND_TAG_INFO_SELF, Placeholder.of("who", target.name()));
				}
			}
		}
	}
